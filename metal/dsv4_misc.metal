...
/* ROUTER-REG8.  The shipped selector runs k_used FULL SCANS: for each of the
 * eight ranks it re-walks all ~9 of this lane's entries, re-reduces across the
 * simdgroup, and retires one winner.  That is 8 x 9 threadgroup reads plus the
 * `taken` bookkeeping, to extract 8 of 288.
 *
 ...
 *
 * WHY (float,int) PAIRS AND NOT A PACKED 64-BIT KEY.  The obvious trick is to
 * pack score and index into one sortable uint64 and let the hardware compare
 * it.  Apple GPUs do not have fast 64-bit integer compare, and the packing
 * would also have to handle -0.0f and NaN by hand.  Two 32-bit shuffles per
 * element is the cheaper and safer shape here.
 *
 * ORDERING IS THE SHIPPED ONE, byte for byte: score-descending, index-ascending
 * on ties, via the same predicate as ds4_glm_router_better.  The normalisation
 * loop below is copied verbatim, summation order included, so the weights are
 * bit-identical and not merely equivalent.
 *
...
// K-register-resident scorer.  The staged K tile is invariant across the head
// loop, yet the threadgroup-staged variant reloads it for every head.  Here
// each simdgroup loads its sixteen 8x8 K tiles into simdgroup registers once
// before the head loop, so per head only the two Q tiles are loaded and
// threadgroup loads per MMA halve.  Staging, barriers and the per-pair
// reduction order are unchanged, so the output is bit-identical.  The cost is
// ~16 live matrix registers per simdgroup across the head loop.
...
kernel void kernel_dsv4_indexer_scores_llt_impl(
    ...
    /* sk is written once and read once -- every simdgroup transposes its
