...
/* Shared body of the streaming top-k selector: one pass over a score row,
 * keeping a running k-th-best threshold so a candidate is discarded only when
...
static inline void ds4_topk_stream_core(
    ...
    for (uint base = 0; base < count; base += TILE) {
        ...
        if (i < count) {
            ...
            key = ds4_topk_pack_key(frow[g], g);
            take = key > thr;
        }
