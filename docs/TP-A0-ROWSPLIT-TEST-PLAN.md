...
| **B** | **`DS4_METAL_GPU_ENCODER_TIMESTAMPS=1`** at 2k and 131k. **Build ≥ `d50bbaa`.** | **Still the one that matters.** Re-baselines the budget at 1–3% distortion instead of 18%, in ~175 spans instead of 14. **Gate: where it disagrees with the hand-corrected table by >0.3 ms on any stage, believe it and re-rank before writing code.** | **RE-RUN DONE 2026-08-27** (build `0b987e5`, tick-calibrated). Tick = **0.632-0.642 ns @2k** (not 1.0 — confirms the unit error), **coverage ~198% OVERLAPPING** at both contexts (spans genuinely overlap ~2×, so per-encoder figures are upper bounds). Calibrated `reduce`: 199 µs/call @2k (41/token), 311 µs/call @131k (20/token). Throughput at baseline (42.04 / 29.70 t/s). True per-encoder busy ≤ ~half the reported spans (~100-155 µs/call reduce). See `BENCHMARKS-TP-PP.md` §Arm B (re-run). |
