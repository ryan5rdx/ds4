...
ifeq ($(UNAME_S),Darwin)
.PHONY: metal-decode-schedule-bench metal-prefill-variant-bench metal-flash-attn-decode-bench check-mxfp4-half-lut check-dispatch-count test-indexer-scorer test-topk-stream512
...
help:
	...
	@echo "  make test-indexer-scorer  Pin the register-resident prefill scorer bit-identical to the staged one"
	@echo "  make test-topk-stream512  Pin the streaming top-512 selector against the argsort fallback and a CPU reference"
