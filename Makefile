...
tests/test_indexer_scorer: tests/test_indexer_scorer.o ds4_metal.o
	$(CC) $(CFLAGS) -o $@ $^ $(METAL_LDLIBS)

test-indexer-scorer: tests/test_indexer_scorer
...
# definition itself.
# The timeline swizzle (ds4_tl_dispatch*, and the @selector pair that installs
...
check-dispatch-count:
	@calls=$$(grep 'dispatchThreadgroups:\|dispatchThreads:' ds4_metal.m | grep -vc 'ds4_tl_\|@selector'); \
	 ...
	 if [ "$$disp" -ne "$$((calls + 1))" ]; then \
	   ...
	   grep -n 'dispatchThreadgroups:\|dispatchThreads:' ds4_metal.m | grep -v 'ds4_tl_\|@selector' | grep -v 'DS4_DISP('; \
	   exit 1; \
	 fi; \
...
ds4_cli.o: ds4_cli.c ds4.h ds4_ssd.h ds4_distributed.h ds4_tp.h ds4_help.h linenoise.h ds4_prompt_prefix.h
...
ds4_eval.o: ds4_eval.c ds4.h ds4_ssd.h ds4_distributed.h ds4_tp.h ds4_help.h ds4_eval_cases.h
...
ds4_cli_cpu.o: ds4_cli.c ds4.h ds4_ssd.h ds4_distributed.h ds4_tp.h ds4_help.h linenoise.h ds4_prompt_prefix.h
...
ds4_eval_cpu.o: ds4_eval.c ds4.h ds4_ssd.h ds4_distributed.h ds4_tp.h ds4_help.h ds4_eval_cases.h
	$(CC) $(CFLAGS) -DDS4_NO_GPU -c -o $@ ds4_eval.c

ds4_agent_cpu.o: ds4_agent.c ds4.h ds4_ssd.h ds4_distributed.h ds4_tp.h ds4_help.h ds4_kvstore.h ds4_web.h linenoise.h ds4_prompt_prefix.h
...
# Links ds4_ane.o so the REAL request ring is under test, not a stand-in.
tests/probe_ane_handoff: tests/probe_ane_handoff.c ds4_metal.o ds4_ane.o ds4_gpu.h ds4_ane.h
	$(CC) $(CFLAGS) -I. -o $@ tests/probe_ane_handoff.c ds4_metal.o ds4_ane.o $(METAL_LDLIBS)
...
tests/test_engine_mgpu_placement: tests/test_engine_mgpu_placement.o ds4_cpu_test_hooks.o ds4_image.o ds4_distributed_cpu.o ds4_tp.o ds4_ssd.o ds4_layer_pack.o
	$(CC) $(CFLAGS) -o $@ $^ $(LDLIBS)

tests/test_sampling.o: tests/test_sampling.c ds4.h
	$(CC) $(CFLAGS) -fno-finite-math-only -DDS4_TEST_HOOKS -I. -c -o $@ $<

tests/test_sampling: tests/test_sampling.o ds4_cpu_test_hooks.o ds4_image.o ds4_distributed_cpu.o ds4_tp.o ds4_ssd.o ds4_layer_pack.o
	$(CC) $(CFLAGS) -o $@ $^ $(LDLIBS)
...
test: check-threadgroup-memory ds4_test ds4_agent_test ds4-eval q4k-dot-test mxfp4-dot-test test-session-state test-linux-memory \
