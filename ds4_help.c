...
static void print_server_api(FILE *fp, const help_colors *c) {
    ...
    opt(fp, c, "--resident-sessions N", "Keep N sessions warm while serializing decode.");
    opt(fp, c, "--prefill-quantum N", "Prefill tokens per engine call when idle (batched mode only). Default: 4096");
...
static void print_kv_cache(FILE *fp, const help_colors *c) {
    ...
    opt(fp, c, "--tool-memory-file PATH", "Persist the exact-DSML tool map so replay survives a restart.");
