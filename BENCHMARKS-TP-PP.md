...
worker mat) with `--batched-session N`, N ∈ {1,2,4,8}, each driving N
concurrent completions (max_tokens 64, factorial prompt).

| N | mean per-slot decode (t/s) | aggregate (t/s) | wall |
|---|---|---|---|
| 1 | 38.81 | **38.81** | 2.76 s |
| 2 | 18.59 | **37.19** | 4.90 s |
| 4 | 9.79 | **39.14** | 8.90 s |
| 8 | 4.35 | **34.79** | 17.55 s |

**Per-slot collapses 38.8 → 4.35 (−89%) with slot count, but aggregate is FLAT
(37-39 t/s) across all four counts.** Per the plan's read, flat aggregate is
the healthy result — but with a crucial nuance: the aggregate is pinned at the
**single-session decode rate** (38 t/s). Concurrent slots do not add per-batch
overhead; they simply *divide the already-fully-utilised decode path*.

**Batching behaviour observed: no coalescing benefit, but no per-batch penalty
either.** Each slot takes its own encode and the GPU decode path is already the
bottleneck at ~38 t/s aggregate, so N slots each get 38/N. There is NO per-slot
collapse beyond simple division — the W1/W1b per-batch cost is not amplified by
slot count in the aggregate. Multi-slot serving is bounded by the same
~38 t/s single-session decode ceiling, not by a repeated per-batch encode cost.

**Verdict.** The multi-slot exposure is benign: aggregate throughput is flat and
conserved, so the per-batch cost W1 found does not stack across slots. The
true ceiling on multi-slot serving is the single-session decode rate (38 t/s),
which is the same decode bottleneck every arm has hit. Multi-slot batching is
not the path to more tokens/s on this rig — the decode path itself is.
