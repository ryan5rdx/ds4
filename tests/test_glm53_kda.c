 ...
 * tiled kernel -- both the banked KREG path and the pre-bank loop that
 * DS4_METAL_IDXPORT=original restores -- with no numeric coverage. This covers
...
int main(void) {
     ...
     * heads and so never crosses the tiled gate. The banked KREG path is the
     * default; DS4_METAL_IDXPORT=original selects the pre-bank loop and is read
     * once per process, so covering both means running this binary twice. */
